...
class OrderController {
  ...
  async store({ request, response }) {
    try {
      ...
      const order = await Database.raw(
        `
      INSERT INTO orders (f_name, l_name, address, address_2, city, state, zipcode, country, payment_type, user_id)
      VALUES(${sanitize.escape(post.form.f_name)},
      ...
      `
        ...
        VALUES(${sanitize.escape(item.productInfo.title)},
  ...
  async edit({ view, request, response, params }) {
    try {
      ...
      return view.render("admin/orders/edit", { orderInfo });
    } catch (error) {
      console.log(error);
      return response.redirect("back");
    }
