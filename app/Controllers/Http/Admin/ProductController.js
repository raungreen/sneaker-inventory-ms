...
class ProductController {
  async index({ view, request, response }) {
    try {
      ...
      // `<h1 style= "color: red"> Something has gone wrong check your code! </h1>
  ...
  async store({ request, response }) {
    try {
      ...
      await Database.raw(
        `
      INSERT INTO products (title, sku, price, img_url, material, description, brand_id, qty, size, user_id)
  ...
  async show({ view, request, response, params }) {
    try {
      let product = await Database.raw(`
        SELECT products.id, products.title, products.sku, products.price, products.img_url, products.description,
        brands.title as brand, CONCAT(users.f_name, ' ', users.l_name) as user, 
        products.material, products.qty, products.size, 
        products.user_id, products.created_at 
        FROM products
  ...
  async sendAllProducts({ view, request, response }) {
    try {
      let allProducts = await Database.raw(`
        SELECT products.id, products.title, products.sku, products.price, products.description, products.img_url,
        brands.title as brand, CONCAT(users.f_name, ' ', users.l_name) as user, 
